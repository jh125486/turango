...
func TestSubtotal(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"one line": {
			items: []Item{{Name: "mug", UnitCents: 1250, Quantity: 2}},
			want:  2500,
		},
		"several lines": {
			items: []Item{
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if got := Subtotal(tt.items); got != tt.want {
...
func TestDiscountCents(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"half off":                   {subtotal: 2000, coupon: CouponHalfOff, want: 1000},
		"ten off a large order":      {subtotal: 2500, coupon: CouponTenOff, want: 1000},
		"ten off a small order":      {subtotal: 999, coupon: CouponTenOff, want: 999},
		"member discount for member": {subtotal: 2000, coupon: CouponMember, member: true, want: 200},
	}
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if got := DiscountCents(tt.subtotal, tt.coupon, tt.member); got != tt.want {
...
func TestShippingCents(t *testing.T) {
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if got := ShippingCents(tt.subtotal, tt.expedited); got != tt.want {
...
func TestTotal(t *testing.T) {
	...
	basket := []Item{{Name: "mug", UnitCents: 3000, Quantity: 2}}
	...
	tests := map[string]struct {
		...
		"free shipping, no coupon": {
			order: Order{Items: basket},
			want:  6000,
		},
		"a discount can push the order back under the free-shipping threshold": {
			order: Order{Items: basket, Coupon: CouponHalfOff},
			want:  3599,
		},
		"expedited always pays": {
			order: Order{Items: basket, Expedited: true},
			want:  7799,
		},
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if got := Total(tt.order); got != tt.want {
...
func TestRestockingFeeCents(t *testing.T) {
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if got := RestockingFeeCents(tt.subtotal); got != tt.want {
